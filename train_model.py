...
#    early_stopping = keras.callbacks.EarlyStopping(monitor='val_loss', patience=2, verbose=2)
#    val_inputs, val_outputs = np.load(sys.argv[3]), np.load(sys.argv[4])
    # Now train the model
    model.fit(inputs, outputs, epochs=5, batch_size=100)
#    model.fit(inputs, outputs, epochs=10, batch_size=64, callbacks=[early_stopping],
#                validation_data=(val_inputs, val_outputs), shuffle=True)
